...
static const char kernel_src[] =
    "                                                                          "
    "                              "
    "extern \"C\"  __global__ void sum(unsigned int* array, unsigned  int* "
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int i = 0; i < SIZE; i++) inputData[i] = 1;

  // Create a buffer
  buffer outbuffer =
      cudaDevice.create_buffer(SIZE * sizeof(unsigned int)).get();

  // Copy input data to the buffer
  data_futures.push_back(
      outbuffer.enqueue_write(0, SIZE * sizeof(unsigned int), inputData));
  ...
  data_futures.push_back(
      resbuffer.enqueue_write(0, sizeof(unsigned int), result));
  ...
  data_futures.push_back(
      lengthbuffer.enqueue_write(0, sizeof(unsigned int), n));
  ...
  auto kernel_future = prog.run(args, "sum", grid, block, 0);

  hpx::wait_all(kernel_future);

  // Copy the result back
  unsigned int* res =
      resbuffer.enqueue_read_sync<unsigned int>(0, sizeof(unsigned int));
