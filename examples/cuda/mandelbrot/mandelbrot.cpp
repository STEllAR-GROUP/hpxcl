...
int main(int argc, char* argv[]) {
  ...
  for (size_t it = 0; it < iterations; it++) {
    ...
    for (int j = 0; j < numDevices; j++) {
      ...
      mode.append(
          std::to_string(devices[j].get_device_architecture_major().get()));
      mode.append(
          std::to_string(devices[j].get_device_architecture_minor().get()));
    ...
    for (int j = 0; j < numDevices; j++) {
      ...
      data_futures[j].push_back(
          heightBufferVector[j].enqueue_write(0, sizeof(int), &currentHeight));
      yStartBufferVector.push_back(bufferFutures[j][3].get());
      data_futures[j].push_back(
          yStartBufferVector[j].enqueue_write(0, sizeof(int), &yStart[j]));
      ...
      kernelFutures.push_back(
          progVector[j].run(args, "kernel", grid, block, args));
#else
      kernelFutures.push_back(progVector[j].run(args, "kernel", grid, block));
#endif
      // for multiple runs
      args.clear();
    }
    ...
    for (int j = 0; j < numDevices; j++) {
      image = imageBufferVector.at(j).enqueue_read_sync<char>(
          0, bytes / numDevices);
      std::copy(
          image, image + currentWidth * (currentHeight / numDevices) * 3 - 1,
          mainImage + currentWidth * (currentHeight / numDevices) * 3 * j);
    }
    img_data =
        std::make_shared<std::vector<char>>(mainImage, mainImage + bytes);

    writeImages.push_back(
        hpx::async(save_png_it, img_data, currentWidth, currentHeight, it));
