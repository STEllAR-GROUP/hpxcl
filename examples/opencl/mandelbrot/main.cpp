...
void grainsize_bench(std::vector<hpx::opencl::device> devices, size_t img_x,
                     size_t img_y, size_t num_iterations, size_t num_kernels,
                     bool verbose) {
  ...
  for (size_t grainsize = 256; grainsize <= img_x * img_y; grainsize *= 2) {
    ...
    hpx::cerr << "Using tilesizes: " << tilesize_x << "x" << tilesize_y
              << hpx::endl;
    ...
    for (size_t i = 0; i < num_iterations + 1; i++) {
      ...
      img_gen
          .compute_image(posx, posy, zoom, 0.0, img_x, img_y, true, tilesize_x,
                         tilesize_y)
          .get();
    ...
    hpx::cout << grainsize << "\t" << time << hpx::endl;
  }
...
void speedup_bench(std::vector<hpx::opencl::device> devices, size_t tilesize_x,
                   size_t tilesize_y, size_t img_x, size_t img_y,
  ...
  for (size_t num_gpus = 1; num_gpus <= devices.size(); num_gpus++) {
    hpx::cerr << "Starting test with " << num_gpus << " gpus ..." << hpx::endl;

    // Add another worker
    if (verbose) {
      hpx::cerr << "adding worker ..." << hpx::endl;
    }
    img_gen.add_worker(devices[devices.size() - num_gpus], 4);

    // Wait for the worker to initialize
    if (verbose) {
      hpx::cerr << "waiting for worker to finish startup ..." << hpx::endl;
    }
    img_gen.wait_for_startup_finished();
    ...
    if (num_gpus == 1) single_gpu_time = time;

    hpx::cerr << "Time: " << time << " ms" << hpx::endl;
    hpx::cout << num_gpus << "\t" << time << "\t" << (single_gpu_time / time)
...
int hpx_main(boost::program_options::variables_map& vm) {
  ...
  if (vm.count("bench")) benchmark = true;
  ...
  // The main scope
  {
    ...
    std::vector<hpx::opencl::device> devices =
        hpx::opencl::create_all_devices(
            CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_ACCELERATOR, "OpenCL 1.1")
            .get();
    ...
    if (!benchmark) {
      ...
      std::shared_ptr<std::vector<char>> img_data =
          img_gen
              .compute_image(posx, posy, zoom, 0.0, img_x, img_y, false, img_x,
                             4)
              .get();
      ...
      /*speedup_bench(devices,
                    ...
                    verbose);
*/
      grainsize_bench(devices, 2048, 1024, 10, 4, verbose);
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  cmdline.add_options()("v", "verbose output");

  cmdline.add_options()("bench", "runs benchmark");
