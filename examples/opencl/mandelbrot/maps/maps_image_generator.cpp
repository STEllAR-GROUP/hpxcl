...
maps_image_generator::maps_image_generator(
    size_t img_size_hint_x_, size_t img_size_hint_y_,
    size_t num_parallel_kernels, bool verbose_,
    boost::function<std::shared_ptr<request>(void)> acquire_new_request_,
...
void maps_image_generator::dispose_current_request_if_invalid() {
  if (current_request) {
    if (!current_request->stillValid()) {
      ...
      {
        // lock the data list
        boost::lock_guard<hpx::lcos::local::spinlock> lock2(images_lock);

        // insert new request to images list
        images.erase(current_request_id);
      }

      current_request = std::shared_ptr<request>();
...
bool maps_image_generator::worker_request_new_work(
    std::shared_ptr<workload>* new_work) {
  // lock
  boost::lock_guard<hpx::lcos::local::spinlock> lock(current_request_lock);
  ...
  if (verbose)
    hpx::cout << "new work aquired. calculating new workload ..." << hpx::endl;

  // calculate current coords
  double workpacket_pos_x =
      current_topleft_x +
      current_vert_pixdist_x * current_img_pos * current_request->lines_per_gpu;
  double workpacket_pos_y =
      current_topleft_y +
      current_vert_pixdist_y * current_img_pos * current_request->lines_per_gpu;

  // TODO calculate new workload
  *new_work = std::make_shared<workload>(
      current_request->tilesize_x, current_request->lines_per_gpu,
      workpacket_pos_x, workpacket_pos_y, current_hor_pixdist_x,
      current_hor_pixdist_y, current_vert_pixdist_x, current_vert_pixdist_y,
      current_request_id, 0, current_img_pos * current_request->lines_per_gpu,
...
void maps_image_generator::start_getting_new_image() {
  hpx::async(&maps_image_generator::get_new_image, this)
      .then(hpx::util::bind(

          // if no new image can be fetched, shut down generator
          [](maps_image_generator* img_gen,
             hpx::lcos::future<bool> parent_future) {
            if (!parent_future.get()) img_gen->shutdown();
          },

          this, hpx::util::placeholders::_1

          ));
}

void maps_image_generator::worker_deliver(
    std::shared_ptr<workload>& done_work) {
...
void maps_image_generator::add_worker(hpx::opencl::device& device,
  ...
  boost::function<bool(std::shared_ptr<workload>*)> request_new_work =
      boost::bind(&maps_image_generator::worker_request_new_work, this, _1);

  // create deliver callback function for worker
  boost::function<void(std::shared_ptr<workload>&)> deliver_done_work =
      boost::bind(&maps_image_generator::worker_deliver, this, _1);
...
bool maps_image_generator::get_new_image() {
  ...
  // get new request
  std::shared_ptr<request> new_request = acquire_new_request();
  ...
  // allocate image data
  new_request->data = std::make_shared<std::vector<char>>(
      new_request->tilesize_x * new_request->tilesize_y * 3 * sizeof(char));
...

  {
    ...
    // insert new request to images list
    images.insert(
        std::pair<size_t, std::shared_ptr<request>>(new_image_id, new_request));
  }
  ...
  hpx::cout << "Started working on " << current_request->zoom << " - (" << posx
            << ", " << posy << ")" << hpx::endl;

  return true;
}

void maps_image_generator::wait_for_startup_finished() {
...
void maps_image_generator::shutdown() {
...
void maps_image_generator::retrieve_worker_main(intptr_t parent_,
                                                bool verbose) {
  // get parent pointer
  ...
  while (parent->done_work_queue.pop(&done_workload)) {
    if (verbose)
      hpx::cout << "retrieved workload " << done_workload->pos_in_img_x << ":"
    ...
    // image data
    std::shared_ptr<request> img_request;

    // retrieve image pointers
    {
      // lock
      boost::lock_guard<hpx::lcos::local::spinlock> lock(parent->images_lock);
      ...
      if (req_iterator == parent->images.end()) continue;
    ...
    for (size_t y = 0; y < size_y; y++) {
      for (size_t x = 0; x < size_x; x++) {
        (*(img_request
               ->data))[((y + start_y) * line_offset + (x + start_x)) * 3 + 0] =
            done_workload->pixeldata[(y * size_x + x) * 3 + 0];
        (*(img_request
               ->data))[((y + start_y) * line_offset + (x + start_x)) * 3 + 1] =
            done_workload->pixeldata[(y * size_x + x) * 3 + 1];
        (*(img_request
               ->data))[((y + start_y) * line_offset + (x + start_x)) * 3 + 2] =
    ...
    if (current_img_countdown == 0) {
      ...
      if (img_request->stillValid()) {
        ...
        boost::shared_array<char> png_data =
            create_png(img_request->data, img_request->tilesize_x,
                       img_request->tilesize_y, &png_size);

        // remove old data
        img_request->data = std::make_shared<std::vector<char>>(
            png_data.get(), png_data.get() + png_size);
      ...
      if (img_it != parent->images.end()) parent->images.erase(img_it);
    }
  }
}
...
hpx::endl;
    ...
    std::shared_ptr<std::atomic_size_t> img_countdown =
              std::make_shared<std::atomic_size_t>(num_tiles_x * num_tiles_y);
    ...
    for(size_t y = 0; y < img_height; y += tile_height)
    {
        ...
        {
            if (verbose) hpx::cout << "\tAdding workload " << x << ":" << y << "
..." << hpx::endl;

            // calculate position of current work packet
            double workpacket_pos_x = topleft_x + vert_pixdist_x * y +
hor_pixdist_x * x; double workpacket_pos_y = topleft_y + vert_pixdist_y * y +
hor_pixdist_y * x;
            // add workload
