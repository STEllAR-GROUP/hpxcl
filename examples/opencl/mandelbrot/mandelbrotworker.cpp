...
mandelbrotworker::mandelbrotworker(
    ...
    boost::function<bool(std::shared_ptr<workload>*)> request_new_work_,
    boost::function<void(std::shared_ptr<workload>&)> deliver_done_work_,
    bool verbose_, size_t workpacket_size_hint_x, size_t workpacket_size_hint_y)
    : verbose(verbose_),
      id(id_counter++),
      device(device_),
      worker_initialized(std::make_shared<hpx::lcos::local::event>()),
      request_new_work(request_new_work_),
      deliver_done_work(deliver_done_work_) {
  // start worker
  worker_finished =
      hpx::async(&mandelbrotworker::worker_starter, this, num_workers,
                 workpacket_size_hint_x, workpacket_size_hint_y);
}
...
size_t mandelbrotworker::worker_main(hpx::opencl::kernel precalc_kernel,
  ...
  mandelbrotworker_buffermanager buffermanager(
      device,
      workpacket_size_hint_x * workpacket_size_hint_y * 3 * sizeof(char),
      verbose, CL_MEM_WRITE_ONLY);

  // initialize buffermanager for precalc buffer
  mandelbrotworker_buffermanager precalc_buffermanager(
      device,
      (workpacket_size_hint_x + 2) * (workpacket_size_hint_y + 2) *
          sizeof(char),
      verbose, CL_MEM_READ_WRITE);
  ...
  hpx::opencl::buffer output_buffer =
      buffermanager.get_buffer(current_buffer_size);

  // attach precalc buffer
  size_t current_precalc_size = (workpacket_size_hint_x + 2) *
                                (workpacket_size_hint_y + 2) * sizeof(char);
  hpx::opencl::buffer precalc_buffer =
      precalc_buffermanager.get_buffer(current_precalc_size);

  // create input buffer
  hpx::opencl::buffer input_buffer = device.create_buffer(
      CL_MEM_READ_ONLY, KERNEL_INPUT_ARGUMENT_COUNT * sizeof(double));
  ...
  // main loop
  std::shared_ptr<workload> next_workload;
  ...
  while (request_new_work(&next_workload)) {
    ...
    size_t needed_buffer_size = next_workload->num_pixels_x *
                                next_workload->num_pixels_y * 3 * sizeof(char);
    ...
    if (current_precalc_size != needed_precalc_size) {
      // query new buffer
      precalc_buffer = precalc_buffermanager.get_buffer(needed_precalc_size);
...
void mandelbrotworker::worker_starter(size_t num_workers,
                                      size_t workpacket_size_hint_x,
                                      size_t workpacket_size_hint_y) {
  try {
    std::string device_vendor =
        device.get_device_info<CL_DEVICE_VENDOR>().get();
    std::string device_name = device.get_device_info<CL_DEVICE_NAME>().get();
    std::string device_version =
        device.get_device_info<CL_DEVICE_VERSION>().get();

    // print device name
    hpx::cerr << "#" << id << ": " << device_vendor << ": " << device_name
              << " (" << device_version << ")" << hpx::endl;
    ...
    for (size_t i = 0; i < num_workers; i++) {
      ...
      hpx::lcos::future<size_t> worker_future =
          hpx::async(&mandelbrotworker::worker_main, this, precalc_kernel,
                     kernel, workpacket_size_hint_x, workpacket_size_hint_y);
    ...
    if (verbose) {
      hpx::cout << "#" << id << ": "
                << "workers finished! (" << num_work << " work packets)"
                << hpx::endl;
    }
