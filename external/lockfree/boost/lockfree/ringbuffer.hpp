...
class ringbuffer_base : boost::noncopyable {
  ...
  static const int padding_size =
      BOOST_LOCKFREE_CACHELINE_BYTES - sizeof(size_t);
 ...
 protected:
  ringbuffer_base(void) : write_index_(0), read_index_(0) {}

  static size_t next_index(size_t arg, size_t max_size) {
    size_t ret = arg + 1;
    while (unlikely(ret >= max_size)) ret -= max_size;
    return ret;
  }
  ...
  static size_t write_available(size_t write_index, size_t read_index,
    ...
    if (write_index >= read_index) ret += max_size;
    return ret;
  }
  ...
  size_t enqueue(const T* input_buffer, size_t input_count, T* internal_buffer,
    ...
    if (write_index + input_count > max_size) {
      ...
      if (new_write_index == max_size) new_write_index = 0;
    }
  ...
  bool dequeue(T& ret, T* buffer, size_t max_size) {
    ...
    if (empty(write_index, read_index)) return false;
  ...
  size_t dequeue(T* output_buffer, size_t output_count,
    ...
    if (read_index + output_count > max_size) {
      ...
      std::copy(internal_buffer + read_index, internal_buffer + max_size,
                output_buffer);
      std::copy(internal_buffer, internal_buffer + count1,
      ...
      if (new_read_index == max_size) new_read_index = 0;
    }
...
class ringbuffer : public detail::ringbuffer_base<T> {
  typedef std::size_t size_t;
 ...
 public:
  ...
  size_t enqueue(T const* t, size_t size) {
    return detail::ringbuffer_base<T>::enqueue(t, size, array_.c_array(),
                                               max_size);
  }
  ...
  /** Enqueues size objects from the iterator range [begin, end[ to the
   * ringbuffer.
   *
   *  Enqueueing may fail, if the ringbuffer is full.
   *
  ...
  template <typename ConstIterator>
  ConstIterator enqueue(ConstIterator begin, ConstIterator end) {
    return detail::ringbuffer_base<T>::enqueue(begin, end, array_.c_array(),
                                               max_size);
  }
  ...
  size_t dequeue(T* ret, size_t size) {
    return detail::ringbuffer_base<T>::dequeue(ret, size, array_.c_array(),
                                               max_size);
  }
...
template <typename T>
class ringbuffer<T, 0> : public detail::ringbuffer_base<T> {
  typedef std::size_t size_t;
  size_t max_size_;
 ...
 public:
   ...
   */
  size_t enqueue(T const* t, size_t size) {
    return detail::ringbuffer_base<T>::enqueue(t, size, array_.get(),
                                               max_size_);
  }
  ...
  template <typename ConstIterator>
  ConstIterator enqueue(ConstIterator begin, ConstIterator end) {
    return detail::ringbuffer_base<T>::enqueue(begin, end, array_.get(),
                                               max_size_);
  }
   ...
   * */
  size_t dequeue(T* ret, size_t size) {
    return detail::ringbuffer_base<T>::dequeue(ret, size, array_.get(),
                                               max_size_);
  }
