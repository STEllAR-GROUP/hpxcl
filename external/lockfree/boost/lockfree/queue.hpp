...
class queue : boost::noncopyable {
 private:
  ...
  struct BOOST_LOCKFREE_CACHELINE_ALIGNMENT node {
    ...
    node(void) : next(tagged_node_ptr(nullptr, 0)) {}
  ...
  typedef typename boost::mpl::if_<
      ...
      detail::freelist_stack<node, false, node_allocator> >::type pool_t;
 ...
 public:
   ...
   *                      lockfree, if this is true. Using c++0x-style atomics,
   * there is no possibility to provide a completely accurate implementation,
   * because one would need to test every internal node, which is impossible if
   * further nodes will be allocated from the operating system.
   * */
  ...
  void reserve(std::size_t n) { pool.reserve(n); }

  //! \copydoc boost::lockfree::stack::reserve_unsafe
  void reserve_unsafe(std::size_t n) { pool.reserve_unsafe(n); }
   ...
   * */
  bool empty(void) { return head_.load().get_ptr() == tail_.load().get_ptr(); }
      ...
      if (likely(tail == tail2)) {
        if (next_ptr == 0) {
          if (tail->next.compare_exchange_weak(
                  next, tagged_node_ptr(n, next.get_tag() + 1))) {
            tail_.compare_exchange_strong(
                tail, tagged_node_ptr(n, tail.get_tag() + 1));
            return true;
          }
        } else
          tail_.compare_exchange_strong(
              tail, tagged_node_ptr(next_ptr, tail.get_tag() + 1));
      }
    }
  }
      ...
      //             if (likely(head == head2))
      {
        if (head.get_ptr() == tail.get_ptr()) {
          ...
          if (head_.compare_exchange_weak(
                  head, tagged_node_ptr(next_ptr, head.get_tag() + 1))) {
 ...
 private:
  ...
  static const int padding_size =
      BOOST_LOCKFREE_CACHELINE_BYTES - sizeof(tagged_node_ptr);
 ...
 *  from the operating system, and struct static_freelist_t uses a fixed-sized
 * freelist. With a fixed-sized freelist, the push operation may fail, while
 * with a caching freelist, the push operation may block.
 *
 *  \b Limitation: The class T is required to have a trivial assignment
 * operator.
 *
 * */
template <typename T, typename freelist_t = caching_freelist_t,
          typename Alloc = std::allocator<T> >
class queue : public detail::queue<T, freelist_t, Alloc> {
 ...
 public:
  ...
  //! Construct queue, allocate n nodes for the freelist.
  explicit queue(std::size_t n) : detail::queue<T, freelist_t, Alloc>(n) {}
};
...
class queue<T*, freelist_t, Alloc>
 ...
 public:
  ...
  //! Construct queue, allocate n nodes for the freelist.
  explicit queue(std::size_t n) : queue_t(n) {}

  //! \copydoc detail::queue::pop
  bool pop(T*& ret) { return queue_t::pop(ret); }
   ...
   *
   * */
  bool pop(std::auto_ptr<T>& ret) { return pop_smart_ptr(ret); }
   ...
   *
   * */
  bool pop(std::shared_ptr<T>& ret) { return pop_smart_ptr(ret); }
};
