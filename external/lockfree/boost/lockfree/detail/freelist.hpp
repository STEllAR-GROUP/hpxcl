...
template <typename T, bool allocate_may_allocate,
          typename Alloc = std::allocator<T> >
class freelist_stack : Alloc {
  typedef tagged_ptr<freelist_node> tagged_node_ptr;

 public:
  freelist_stack(std::size_t n = 0) : pool_(tagged_node_ptr(nullptr)) {
    reserve_unsafe(n);
  }
  ...
  void deallocate(T* n) {
    ...
    for (;;) {
      ...
      if (pool_.compare_exchange_weak(old_pool, new_pool)) return;
    }
  }
  ...
  ~freelist_stack(void) {
    ...
    while (current) {
      ...
      if (current_ptr) current = current_ptr->next;
