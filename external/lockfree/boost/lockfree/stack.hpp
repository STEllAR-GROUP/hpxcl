...
class stack : boost::noncopyable {
 ...
 public:
  ...
  //! Construct stack.
  stack(void) : tos(tagged_node_ptr(nullptr, 0)) {}

  //! Construct stack, allocate n nodes for the freelist
  explicit stack(std::size_t n) : tos(tagged_node_ptr(nullptr, 0)) {
  ...
  bool pop(T& ret) {
    ...
    for (;;) {
      if (!old_tos.get_ptr()) return false;
   ...
   * it only guarantees that at some point during the execution of the function
   * the stack has been empty
   * */
  bool empty(void) const { return tos.load().get_ptr() == nullptr; }
