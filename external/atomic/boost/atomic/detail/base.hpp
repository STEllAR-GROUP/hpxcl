...
class internal_atomic<T, Size, void>
    : private detail::atomic::platform_atomic<T> {
 public:
  ...
  bool compare_exchange_strong(T &expected, T desired,
    ...
    return super::compare_exchange_strong(expected, desired, success_order,
                                          failure_order);
  }
...
class internal_atomic<T, Size, int>
 ...
 public:
  ...
  integral_type operator&=(integral_type c) volatile {
    return fetch_and(c) & c;
  }
  integral_type operator|=(integral_type c) volatile { return fetch_or(c) | c; }
  integral_type operator^=(integral_type c) volatile {
    return fetch_xor(c) ^ c;
  }

  integral_type operator+=(integral_type c) volatile {
    return fetch_add(c) + c;
  }
  integral_type operator-=(integral_type c) volatile {
    return fetch_sub(c) - c;
  }
  ...
  bool compare_exchange_strong(
      integral_type &expected, integral_type desired,
      memory_order order = memory_order_seq_cst) volatile {
    return super::compare_exchange_strong(expected, desired, order,
                                          calculate_failure_order(order));
  }
