...
class build_exchange : public Base {
 public:
  ...
  bool compare_exchange_strong(integral_type &expected, integral_type desired,
    ...
    while (true) {
      if (compare_exchange_weak(expected, desired, success_order,
                                failure_order))
        return true;
  ...
  integral_type exchange(integral_type replacement,
    ...
    do {
        ...
        !compare_exchange_weak(o, replacement, order, memory_order_relaxed));
    return o;
  }
...
class build_const_fetch_add : public Base {
 public:
  ...
  integral_type fetch_add(integral_type c,
                          memory_order order = memory_order_seq_cst) volatile {
    if (__builtin_constant_p(c)) {
      switch (c) {
        case -1:
          return fetch_dec(order);
        case 1:
          return fetch_inc(order);
      }
    }
...
class build_fetch_add : public Base {
 public:
  ...
  integral_type fetch_add(integral_type c,
    ...
    do {
      n = o + c;
    } while (!compare_exchange_weak(o, n, order, memory_order_relaxed));
    return o;
  }
...
class build_arithmeticops : public Base {
 public:
  ...
  integral_type fetch_sub(integral_type c,
                          memory_order order = memory_order_seq_cst) volatile {
...
class build_atomic_from_minimal
    : public build_logicops<
          build_arithmeticops<build_fetch_add<build_exchange<Base> > > > {
 public:
  typedef build_logicops<
      build_arithmeticops<build_fetch_add<build_exchange<Base> > > >
      super;
...
class build_atomic_from_typical
    : public build_logicops<
          build_arithmeticops<build_const_fetch_add<Base> > > {
 public:
  typedef build_logicops<build_arithmeticops<build_const_fetch_add<Base> > >
      super;
  typedef typename super::integral_type integral_type;
...
class build_base_from_larger_type {
 public:
  ...
  bool compare_exchange_weak(integral_type &expected, integral_type desired,
    ...
    bool success = get_base().compare_exchange_weak(
        expected_, desired_, success_order, failure_order);
 ...
 private:
  ...
  intptr_t get_shift(void) const volatile {
    ...
    return (sizeof(larger_integral_type) - sizeof(integral_type) -
            get_offset()) *
           8;
#else
...
class build_atomic_from_larger_type
    : public build_atomic_from_minimal<
          build_base_from_larger_type<Base, Type> > {
 public:
  typedef build_atomic_from_minimal<build_base_from_larger_type<Base, Type> >
      super;
  // typedef typename super::integral_type integral_type;
  typedef Type integral_type;
