...
class platform_atomic_integral<T, 1>
    : public build_atomic_from_add<atomic_x86_8<T> > {
 public:
...
class platform_atomic_integral<T, 2>
    : public build_atomic_from_add<atomic_x86_16<T> > {
 public:
...
class platform_atomic_integral<T, 4>
    : public build_atomic_from_add<atomic_x86_32<T> > {
 public:
...
class atomic_x86_64 {
 ...
 public:
  ...
  T exchange(T r, memory_order order = memory_order_seq_cst) volatile {
    T prev = i;
    do {
    } while (!compare_exchange_strong(prev, r, order, memory_order_relaxed));
    return prev;
  }
  ...
  T load(memory_order order = memory_order_seq_cst) const volatile {
    ...
    T expected = i;
    do {
    } while (!const_cast<this_type *>(this)->compare_exchange_strong(
...
class platform_atomic_integral<T, 8>
    : public build_atomic_from_add<atomic_x86_64<T> > {
 public:
...
#if __BOOST_AMD_64 && defined(BOOST_ATOMIC_HAVE_SSE2) && \
...
class platform_atomic_integral<T, 16>
    : public build_atomic_from_add<atomic_x86_128<T> > {
 public:
