...
//
class zerocopy_buffer : public hpx::serialization::serialize_buffer<char> {
  typedef hpx::serialization::serialize_buffer<char> base_type;

 public:
  zerocopy_buffer() BOOST_NOEXCEPT : pointer_(0),
                                     size_x(0),
                                     size_y(0),
                                     size_z(0) {}
  ...
  zerocopy_buffer(std::uintptr_t p, const hpx::opencl::rect_props& rect,
 ...
 private:
    ...
    for (std::size_t z = 0; z < size_z; z++) {
      for (std::size_t y = 0; y < size_y; y++) {
        ar >> hpx::serialization::make_array(
                  dest_addr + y * stride_y + z * stride_z, size_x);
      }
    }
  }
  ...
  void save(Archive& ar, unsigned int const version) const {
    ...
    ar << size_x << size_y << size_z << stride_y << stride_z << pointer_;
    ar << hpx::serialization::make_array(this->base_type::data(),
                                         this->base_type::size());
  }
