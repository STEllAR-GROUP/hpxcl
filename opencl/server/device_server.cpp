...
static void device_cleanup(uintptr_t command_queue_ptr, uintptr_t context_ptr) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());

  cl_int err;

  cl_command_queue command_queue =
      reinterpret_cast<cl_command_queue>(command_queue_ptr);
...
void device::init(cl_device_id _device_id, bool enable_profiling) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());
  ...
  context = clCreateContext(context_properties, 1, &this->device_id,
...
// Create Command Queue
  ...
  cl_queue_properties queue_properties[] = {
      CL_QUEUE_PROPERTIES, (cl_queue_properties)command_queue_properties,
...
#else
  command_queue =
      clCreateCommandQueue(context, device_id, command_queue_properties, &err);
  cl_ensure(err, "clCreateCommandQueue()");
#endif
}

hpx::serialization::serialize_buffer<char> device::get_device_info(
...
hpx::serialization::serialize_buffer<char> device::get_platform_info(
...
void CL_CALLBACK device::error_callback(const char* errinfo, const void* info,
                                        std::size_t info_size, void* _thisp) {
  device* thisp = (device*)_thisp;
  hpx::cerr << "device(" << thisp->device_id << "): CONTEXT_ERROR: " << errinfo
            << hpx::endl;
}

cl_context device::get_context() { return context; }

cl_device_id device::get_device_id() { return device_id; }

hpx::id_type device::create_buffer(cl_mem_flags flags, std::size_t size) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());

  // Create new buffer
  hpx::id_type buf =
      hpx::components::new_<hpx::opencl::server::buffer>(hpx::find_here())
          .get();
...
hpx::id_type device::create_program_with_source(
    hpx::serialization::serialize_buffer<char> src) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());

  // Create new program
  hpx::id_type prog =
      hpx::components::new_<hpx::opencl::server::program>(hpx::find_here())
          .get();
...
hpx::id_type device::create_program_with_binary(
...
void device::release_event(hpx::naming::gid_type gid) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());
...
void device::delete_event(cl_event event) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());
...
void device::register_event(const hpx::naming::id_type& gid, cl_event event) {
  // Add pair to event_map
  event_map.add(gid, event);
}

cl_event device::retrieve_event(const hpx::naming::id_type& gid) {
  // Get event from event_map
  return event_map.get(gid);
}

cl_command_queue device::get_read_command_queue() { return command_queue; }

cl_command_queue device::get_write_command_queue() { return command_queue; }

cl_command_queue device::get_kernel_command_queue() { return command_queue; }

void device::wait_for_cl_event(cl_event event) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());
  ...
  for (std::size_t i = 0; execution_state != CL_COMPLETE; i++) {
    ...
    hpx::util::yield_while([i]() { return i; });
...
void device::delete_event_data(cl_event event) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());

  // return if no data is registered
  if (!event_data_map.has_data(event)) return;
...
void device::activate_deferred_event(hpx::naming::id_type event_id) {
  // get the cl_event
...
void device::activate_deferred_event_with_data(hpx::naming::id_type event_id) {
  // get the cl_event
  cl_event event = event_map.get(event_id);
