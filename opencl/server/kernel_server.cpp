...
kernel::~kernel() {
  ...
  hpx::threads::async_execute(exec, &kernel_cleanup,
                              reinterpret_cast<uintptr_t>(kernel_id))
      .wait();
}

hpx::naming::id_type kernel::get_parent_device_id() { return parent_device_id; }

void kernel::init(hpx::naming::id_type device_id, cl_program program,
...
void kernel::set_arg(cl_uint arg_index, hpx::naming::id_type buffer_id) {
  HPX_ASSERT(hpx::opencl::tools::runs_on_medium_stack());
  cl_int err;
...
void kernel::enqueue(hpx::naming::id_type&& event_gid,
  ...
  err = clEnqueueNDRangeKernel(
      command_queue, kernel_id, static_cast<cl_uint>(size), global_work_offset,
      global_work_size, local_work_size, static_cast<cl_uint>(events.size()),
