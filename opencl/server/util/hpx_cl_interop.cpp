...
void hpx::opencl::server::util ::set_promise_from_external(
    hpx::runtime* rt, hpx::lcos::local::promise<cl_int>* promise,
    cl_int value) {
  ...
  rt->register_thread("opencl",
                      opencl_thread_num.fetch_add(1, std::memory_order_relaxed),
                      false);
...
// This function triggers an hpx::lcos::local::event from an external thread
void hpx::opencl::server::util ::set_promise_from_external(
    hpx::runtime* rt, hpx::lcos::local::promise<void>* promise) {
  // If we are on an hpx thread we don't need any special treatment
  if (rt->get_thread_name() != "<unknown>") {
    promise->set_value();
    return;
