...
static void cl_test(hpx::opencl::device local_device,
  ...
  // test local continuation
  {
    auto data_write_future = buffer.enqueue_write(0, initdata);
    auto future2 =
        data_write_future.then([](hpx::future<void> fut) { return true; });
  ...
  // test remote continuation
  {
    ...
    auto data_read_future = buffer.enqueue_read(0, DATASIZE, data_write_future);
  ...
  // test read continuation and non-char buffer writes and offsets
  {
    ...
    auto data_write_future =
        buffer.enqueue_write(1, modifydata2, data_read_future1);
    auto data_read_future2 =
  ...
  // test buffer-buffer copy local
  {
    ...
    auto futures = buffer.enqueue_send(buffer2, 1, 3, 5, data_write_future,
                                       data_write_future2);
  ...
  // test buffer-buffer copy remote
  {
    ...
    auto futures = buffer.enqueue_send(remote_buffer, 1, 3, 5,
                                       data_write_future, data_write_future2);
