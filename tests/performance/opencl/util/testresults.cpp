...
void testresults::set_enabled_tests(std::vector<std::string> enabled_tests) {
  enabled_tests_set.insert(enabled_tests.begin(), enabled_tests.end());
}

void testresults::set_output_json() { output_format = JSON; }

void testresults::set_output_tabbed() { output_format = TABBED; }

void testresults::start_test(std::string name, std::string unit,
...
void testresults::add(double result) {
  std::cerr << "." << std::flush;
...
bool testresults::needs_more_testing() {
  if (!current_test_valid) return false;

  if (results.back().test_entries.size() >= 10) return false;

  return true;
}

static std::size_t get_column_width(
    const std::vector<std::vector<std::string> >& matrix, std::size_t column) {
  std::size_t max_width = 0;

  for (const auto& row : matrix) {
    if (row.size() <= column) continue;

    if (row[column].length() > max_width) max_width = row[column].length();
  }

  return max_width;
}

static std::string double_to_str(double num) {
  std::stringstream str;
  str << num;
  return str.str();
}

void testresults::print_default(std::ostream& os) const {
...
void testresults::print_tabbed(std::ostream& os) const {
  // print headline
...
void testresults::print_json(std::ostream& os) const {
  os << "{" << std::endl;
  os << "  \"tests\": {" << std::endl;
...
std::ostream& hpx::opencl::tests::performance::operator<<(
    std::ostream& os, const testresults& result) {
  switch (result.output_format) {
    case testresults::DEFAULT:
      result.print_default(os);
      break;
    case testresults::TABBED:
      result.print_tabbed(os);
      break;
    case testresults::JSON:
...
double testresults::testseries::get_min() const {
...
double testresults::testseries::get_max() const {
...
double testresults::testseries::get_stddev() const {
...
double testresults::testseries::get_mean() const {
  double sum = 0;
...
double testresults::testseries::get_median() const {
  double median;
  std::size_t size = test_entries.size();

  if (size == 0) return 0;

  if (size == 1) return test_entries[0];
...
std::string testresults::testseries::get_atts() const {
