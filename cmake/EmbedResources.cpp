...
static bool is_not_alnum(char c) { return !std::isalnum(c, std::locale("C")); }

int main(int argc, const char** argv) {
  // check for correct command line arguments
  if (argc < 3 || argc > 4) {
    std::cout << "Usage: " << argv[0] << " infile outfile [namespace]"
              << std::endl;
    return 2;
  }
  ...
  */
  ...
  while (ifile.good()) {
    ...
    ofile << "'\\x" << std::hex << std::setw(2) << std::setfill('0') << (int)c
          << "',";
    c = ifile.get();
  }
  ofile << "\n};" << std::endl;
  ...
  ofile << "extern const unsigned long " << varname << "_len = " << std::dec
        << std::setw(0) << std::setfill(' ') << numchars << ";" << std::endl;
