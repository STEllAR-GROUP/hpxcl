...
namespace hpx { namespace cuda{ namespace server{ namespace util{
    ...
    public:
        ...
        void add(const hpx::naming::id_type&, cudaEvent_t* event);

        // Retrieves the cl_event associated with the gid.
        // !! BLOCKS if gid is not present until gid gets added
        // with 'add()'.
        cudaEvent_t* get(const hpx::naming::id_type&);
        cudaEvent_t* get(const hpx::naming::gid_type&);

        // Registers a function that will get called upon gid removal
        // (Used to delete associated cl_event)
        void register_deletion_callback(std::function<void(cudaEvent_t* event)> &&);
    ...
    private:
        ...
        typedef std::map<hpx::naming::gid_type, cudaEvent_t*>
            map_type;
        map_type events;
        ...
        std::function<void(cudaEvent_t* event)> deletion_callback;
