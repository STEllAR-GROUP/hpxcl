...
int main(int argc, char *argv[]) {
  ...
  hpx::lcos::future<program> fProg =
      cudaDevice.create_program_with_file("smvp.cu");
  ...
  // creating buffers
  hpx::lcos::future<buffer> fADataBuffer =
      cudaDevice.create_buffer(count * sizeof(double));
  hpx::lcos::future<buffer> fAIndexBuffer =
      cudaDevice.create_buffer(count * sizeof(int));
  hpx::lcos::future<buffer> fAPointerBuffer =
      cudaDevice.create_buffer(m * sizeof(int));

  hpx::lcos::future<buffer> fBBuffer =
      cudaDevice.create_buffer(n * 1 * sizeof(double));
  hpx::lcos::future<buffer> fCBuffer =
      cudaDevice.create_buffer(m * 1 * sizeof(double));
  hpx::lcos::future<buffer> falphaBuffer =
  ...
  hpx::lcos::future<buffer> fcountBuffer =
      cudaDevice.create_buffer(sizeof(int));
  ...
  kernelFutures.push_back(prog.run(args, "smvp", grid, block, 0));
#else
  kernelFutures.push_back(prog.run(args, "smvp", grid, block, 0));
#endif
  ...
  std::cout << validateSmvp(A_data, A_indices, A_pointers, B, res, &m, &n,
                            &count, &alpha)
            << std::endl;
