...
static const char dgemm_src_str[] =
    "                                                                          "
    "\n"
    "   __kernel void dgemm(__global double *A,__global double *B, __global "
    "double *C,__global int *m,__global int *n,__global int *k,__global double "
    "*alpha,__global double *beta)                       \n"
    "   {                                                                      "
    "\n"
...
static buffer_type dgemm_src(dgemm_src_str, sizeof(dgemm_src_str),
...
int main(int argc, char *argv[]) {
  ...
  // creating buffers
  buffer ABuffer =
      cldevice.create_buffer(CL_MEM_READ_ONLY, m[0] * k[0] * sizeof(double));
  buffer BBuffer =
      cldevice.create_buffer(CL_MEM_READ_ONLY, n[0] * k[0] * sizeof(double));
  buffer CBuffer =
      cldevice.create_buffer(CL_MEM_READ_WRITE, m[0] * n[0] * sizeof(double));
  ...
  buffer_data_type A_serialized(A, m[0] * k[0],
                                buffer_data_type::init_mode::reference);

  buffer_data_type B_serialized(B, k[0] * n[0],
                                buffer_data_type::init_mode::reference);

  buffer_data_type C_serialized(C, m[0] * n[0],
                                buffer_data_type::init_mode::reference);

  buffer_data_type alpha_serialized(alpha, 1,
                                    buffer_data_type::init_mode::reference);

  buffer_data_type beta_serialized(beta, 1,
                                   buffer_data_type::init_mode::reference);
  ...
  buffer_parameter_type k_serialized(
      k, 1, buffer_parameter_type::init_mode::reference);
  ...
  dim[0].local_size = 32;
  dim[1].offset = 0;
  dim[1].size = (int)(std::pow(2, std::ceil(std::log(n[0]) / std::log(2))));
  dim[1].local_size = 32;
